            ...
            else
            {
                ...
                if (!qQuery.HaveAny)
                {
                    ...
                    if (useStatis && willNest && !string.IsNullOrEmpty(sColumnName))
                    {
                        ...
                        visitor = new ColumnExpressionVisitor(this, aliases, qQuery.Statis, qQuery.GroupBy, true, sc.Columns);
                        visitor.Write(jf);
                    ...
                    if (qQuery.OrderBy.Count > 0 && useOrderBy && groupByPaging)
                    {
                        ...
                        {
                            visitor = new ColumnExpressionVisitor(this, aliases, qQuery.OrderBy[i], qQuery.GroupBy, true, sc.Columns);
                            visitor.Write(jf);

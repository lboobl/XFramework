...
namespace Renci.SshNet
{
    ...
    public partial class SftpClient : BaseClient
    {
        ...
        /// </remarks>
        public void UploadFile(string fileFullName, string path, Action<ulong> uploadCallback = null)
        {
            Stream stream = null;
            try
            {
                if (!File.Exists(fileFullName)) throw new FileNotFoundException(fileFullName);
                FileInfo fi = new FileInfo(fileFullName);
                stream = fi.OpenRead();
                this.UploadFile(stream, path, uploadCallback);
            }
            catch
            {
                throw;

...
namespace XFramework.Core
{
    public static class EmitMapper
    {
        ...
        public static Func<T,T> MapImpl<T>() where T : class
        {
            DynamicMethod method = new DynamicMethod(string.Format("{0}.Map{1}", typeof(T).FullName, Guid.NewGuid()),
                typeof(T), new Type[] { typeof(T) },typeof(EmitMapper), true);
            ILGenerator il = method.GetILGenerator();
            
            //声明接受复制的实体临时变量
            LocalBuilder b = il.DeclareLocal(typeof(T));
            il.Emit(OpCodes.Newobj, typeof(T).GetConstructor(new Type[] { }));
            il.Emit(OpCodes.Stloc_S, b);

            PropertyInfo[] properties = typeof(T).GetProperties(BINDING_FLAGS_PROPERTY);
            for (int i = 0; i < properties.Length; i++)
            {
                if (!properties[i].CanRead || !properties[i].CanWrite) continue;
                il.Emit(OpCodes.Ldloc_S, b);
                il.Emit(OpCodes.Ldarg_0);
                il.Emit(OpCodes.Callvirt, properties[i].GetGetMethod());
                il.Emit(OpCodes.Callvirt, properties[i].GetSetMethod());
            }
        ...
        static Func<object, Dictionary<Identity, object>, object> CreateCloneMethod1(Type type, Dictionary<Identity, object> objects)
        {
            ...
            if (!type.IsArray)
            {
                ...
                foreach (var field in fields)
                {
                    if (!field.FieldType.IsValueType && field.FieldType != typeof(String))
                    {
                        ...
                        il.Emit(OpCodes.Ldarg_1);
                        il.EmitCall(OpCodes.Call, typeof(EmitMapper).GetMethod("CopyImpl1", BindingFlags.NonPublic | BindingFlags.Static).MakeGenericMethod(field.FieldType), null);
            ...
            else
            {
                ...
                {
                    il.EmitCall(OpCodes.Call, typeof(EmitMapper).GetMethod("CopyImpl1", BindingFlags.NonPublic | BindingFlags.Static).MakeGenericMethod(arraytype), null);
                }
            ...
            if (!type.IsArray)
            {
                ...
                foreach (var prop in props)
                {
                    if (!prop.Type.IsValueType && prop.Type != typeof(String))
                    {
                        ...
                        il.EmitCall(OpCodes.Call, typeof(EmitMapper).GetMethod("CopyImpl2", BindingFlags.NonPublic | BindingFlags.Static).MakeGenericMethod(prop.Type), null);
        ...
        /// 创建对象深度复制的副本
        /// </summary>
        public static T ToObjectCopy<T>(T source) where T : class
        {
            Type type = source.GetType();
        ...
        /// </summary>
        public static void ObjectCopyTo<T>(T source, T target) where T : class
        {
            if (target == null)
        ...
        /// </summary>
        public static void PropCopyTo<T>(T source, T target) where T : class
        {
            if (target == null)
                throw new Exception("将要复制的目标未初始化");
            Type type = typeof(T);

...
namespace ICS.XFramework.Data
{
    ...
    static class DataRecordExtensions
    {
        ...
        /// <returns></returns>
        public static T ToModel<T>(this IDataRecord reader, CommandBase define = null)
        {
            object obj = null;
            object[] values = null;
            int index = 0;
            ...
            var sc = define as CommandDefinition;
            if (sc == null || (sc.NavigationDescriptors != null && sc.NavigationDescriptors.Count == 0))
            {
                // 直接跑SQL,则不解析导航属性
            ...
            {
                // 使用表达式查询，解析导航属性
                bool nav = sc.NavigationDescriptors.Any(x => x.Value.Count > 0);
                int min = nav ? sc.NavigationDescriptors.Min(x => x.Value.Start) : 0;
        ...
        // 导航属性
        private static void ToModel_Navigation(object model, object[] values, CommandDefinition define, string typeName)
        {
            ...
            foreach (var kvp in runtime.NavWrappers)
            {
                ...
                NavigationDescriptor navigation = null;
                define.NavigationDescriptors.TryGetValue(keyName, out navigation);
                ...
                if (navigation.Count > 0)
                {
                    ...
                    {
                        index += 1;
                        if (index < navigation.Start) continue;
                        if (index > (navigation.Start + navigation.Count)) break;
